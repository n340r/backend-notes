...
- Выписать про антифрод, безопасность, базу

- **Пирамида тестирования**: юнит, интеграционные, e2e, нагрузочные, смоук (вроде уже есть чась)
...
- Kafka batch (how messages are sent in practice)

- Extra notes on lock types
  Удалить нахуй, разобраться что из этого реально нужно в той таблице в конспекте

- Когда вообще имеет смысл использовать serializable ?
  Правильно ли я понимаю что даже в транзакции (показать пример из wallarm) где мы забираем аккаунты и переводим деньги, мы используем SELECT FOR UPDATE с read commited ?
  Вопрос про serializable для денег и нужно ли это вообще.
  Слышал read commited используется и row-level lock

- аутентификация OIDC

### ГПТ, посмотри пж есть ли что-то у меня в конспекте про это:
...
- примеры запросов в монге и что они делают просто чтоб ты мог вспомнить или быстро напромприть, я пока не успел у себя выписать
  да тебе гпт расскажет: `$in, $all, $elemMatch, projection, drop, set, deleteMany` - примеры этого
...
- у тебя 50 апи ручек без метрик. Как будешь тестировать какая из них медленная ? Какой-то `slow locks`

- Кэширование на уровне приложения: Nginx, что еще в это входит ? Что еще и почему является этим ?
